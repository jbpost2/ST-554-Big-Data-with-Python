...
- Use `SparkSession` from `pyspark.sql` to create a spark instance (or link to an existing one)
...
- `.read.load()` can read in other delimited data, json data, parquet data, and others
- specific functions like `.read.csv("path")` also exist
...
type(sdf)
```
<pre>pyspark.sql.classic.dataframe.DataFrame</pre>
...
Schema is vital to know (often need to cast to other data types)! (Here `df` is the neuralgia data.)
